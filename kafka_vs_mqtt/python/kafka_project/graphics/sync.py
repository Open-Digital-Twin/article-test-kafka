...
def docker_stats_per_machine(machine, file_list, exp_num, home_dir, exp_type):
    ...
    producer_df_sum.to_csv(f'{file_path}csv/docker_producer_stats_sum_{machine}', index=False)
...
def sum_docker_stats(file_list, exp_num, home_dir, exp_type):
    ...
    producer_df_sum = pd.read_csv(f'{file_path}csv/{producer_files[0]}', header = 0, skiprows=11)
    for file_ in producer_files[1:]:
    ...
    total_df = producer_df_sum.add(consumer_df_sum, fill_value=0)
    total_df.to_csv(f'{file_path}csv/docker_clients_total_stats_sum', index=False)
