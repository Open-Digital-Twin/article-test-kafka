...
for n in range(args.n_times):
    ...
    uid = randint(1, 999)
    if args.swarm == 1:
        container = subprocess.run(['docker', '-H', 'dtwins2', 'ps', '-q', '-f', 'name=kafka_kafka_1'])
        # docker_stats = subprocess.Popen(['docker', '-H dtwins2', 'stats', '8b626307b217', '--format', '"{{.Container}}, {{.CPUPerc}}, {{.MemUsage}}, {{.NetIO}}"'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
