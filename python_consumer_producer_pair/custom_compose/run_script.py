...
for n in range(args.n_times):
    ...
    if args.swarm == 1:
        ...
        subprocess.call(f"docker exec -d $(docker ps -q -f name=kafka_python_consumer) bash -c \"python3 kafka_consumer.py -t {args.topic_name}_{uid}_{n+1} -s kafka_{args.consumer_origin} -p 909{args.consumer_origin} -n {ammount_of_messages}\"", shell=True)
        sleep(5)
        print('Initializing producer...')
        subprocess.call(f"docker exec $(docker ps -q -f name=kafka_python_producer) bash -c \"python3 kafka_producer.py -t {args.topic_name}_{uid}_{n+1} -s kafka_{args.consumer_origin} -p 909{args.producer_destinatary} -n {ammount_of_messages} -d {args.latency} -e {args.entries}\"", shell=True) 
        sleep(2)
        print('waiting for file to be written')
        sleep(2)
        subprocess.call(f"docker cp $(docker ps -q -f name=kafka_python_consumer):/usr/src/app/output_consumer {current_dir}/output_consumer_{uid}_{n+1}", shell=True) 
