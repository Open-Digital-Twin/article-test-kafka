...
for n in range(args.n_times):
    ...
    if args.swarm == 1:
        ...
        docker_stats = subprocess.Popen(['docker', '-H dtwins2', 'stats', container, '--format', '"{{.Container}}, {{.CPUPerc}}, {{.MemUsage}}, {{.NetIO}}"'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
